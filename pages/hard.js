...
export default function Hard() {
  ...
  const queryAPI = (e) => {
    ...
    // make the API call
    axios
      .get(`/api/players/${name.toLowerCase().replace(/ /g, "-")}`)
      .then((res) => {
        ...
        if (res.status === 200) {
          ...
          if (
            guessedPlayers.some(
              (player_object) =>
                player_object.PLAYER_SLUG === name.toLowerCase().replace(/ /g, "-")
            )
          ) {
    ...
    <div className="flex justify-center">
      ...
      <main className="w-[80%] mb-7">
        ...
        <p className="text-gray-300 text-sm sm:text-base text-center pb-7">
          You are currently playing on <span className="text-red-500 italic">hard</span> difficulty. You have <span className="underline">20 minutes</span> to name as many players as you can once you hit start!
